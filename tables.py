...
def table_output(cursor, full_data):
    ...
    table_title = '|'.join([col_names[num].center(width) for num, width in enumerate(max_width)]) + '|'
    print(table_title + '\n' + "-" * len(table_title))

    for data in full_data:
        list_of_data = []
        for d, width in zip(data, max_width):
            if isinstance(d, str | int):
                list_of_data.append(f'{d:{width}}')
                continue
            list_of_data.append(f'{d:{width},.2f}')
